...
<script lang="ts">
	...
	$: formValid = email && passwordValid && passwordsMatch;
...
<svelte:head>
	<title>Inscription - Effervescence 93</title>
</svelte:head>
	...
	<div class="w-full max-w-md">
		{#if !success}
			...
			<div class="text-center mb-8">
				...
				<p class="mt-2 text-gray-600">Rejoignez l'équipe d'administration</p>
			</div>
			...
			<div class="bg-white rounded-lg shadow p-8">
				<form on:submit|preventDefault={handleRegister}>
					...
					<div class="mb-4">
						<label for="password" class="block text-sm font-medium text-gray-700 mb-1">
							...
							{#if password && !passwordValid}
								<span class="text-red-500 text-xs">(min. 8 caractères)</span>
							{/if}
						</label>
						<div class="relative">
							<input
								...
								disabled={loading}
								placeholder="••••••••"
								class="w-full px-3 py-2 pr-10 bg-white border rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
								class:border-gray-300={!password || passwordValid}
					...
					<div class="mb-4">
						<label for="confirm" class="block text-sm font-medium text-gray-700 mb-1">
							...
							{#if confirmPassword && !passwordsMatch}
								<span class="text-red-500 text-xs">(ne correspond pas)</span>
							{/if}
						</label>
						<div class="relative">
							<input
								...
								placeholder="••••••••"
								class="w-full px-3 py-2 pr-10 bg-white border rounded-md text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
								class:border-gray-300={!confirmPassword || passwordsMatch}
								class:border-red-400={confirmPassword && !passwordsMatch}
							/>
							<button
								type="button"
					...
					<!-- Info -->
					<div class="mb-6 p-3 bg-amber-50 border border-amber-200 rounded text-sm text-amber-700">
						Note : Un administrateur devra valider votre compte avant que vous puissiez vous connecter.
					</div>
				...
				<div class="mt-6 pt-6 border-t border-gray-200 text-center">
					...
					{' '}
					<a href="/auth/login" class="text-sm font-medium text-primary-600 hover:text-primary-500">
						Se connecter
					</a>
				</div>
			</div>
